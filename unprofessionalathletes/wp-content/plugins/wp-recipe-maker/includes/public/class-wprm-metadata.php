...
class WPRM_Metadata {
		...
		if ( count( $instruction_groups ) > 0 ) {
			...
			foreach ( $instruction_groups as $group_index => $instruction_group ) {
				...
				foreach ( $instruction_group['instructions'] as $index => $instruction ) {
					...
					// Prevent warning in Structured Data Testing Tool.
					if ( 'howto' === $recipe->type() ) {
					...
					if ( $check_video_parts && isset( $instruction['video'] ) && 'part' === $video_type ) {
						...
						if ( $end > $start ) {
							...
							$url = self::video_get_url_to_time( $metadata['video']['contentUrl'], $start );
							if ( $url ) {
								$video_part_metadata['url'] = $url;
							}
	 ...
	 */
	public static function video_get_url_to_time( $url, $time ) {
		if ( $url ) {
